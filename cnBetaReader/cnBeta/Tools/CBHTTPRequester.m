...
- (void)requestWithURLType:(NSString *)type andId:(NSString *)sid completion:(CompletionBlock)completionBlock
{
    ...
    if ([type isEqualToString:@"content"]){
        ...
        url = @"http://182.92.195.110/api/news?version=2.2.1&init=1";
    }
...
- (void)fetchArticleWithSid:(NSString *)sid useCache:(BOOL) useCache completion:(CompletionBlock)block
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        CBArticle *article = [[CBDataBase sharedDataBase] articleWithSid:sid];
        if (useCache && article.title && article.content) {
            block(article,nil);
            return;
        }
        if (!article) {
