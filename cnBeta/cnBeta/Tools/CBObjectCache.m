...
static NSString *const CBDefaultCacheDirName = @"com.hudy.cnBeta/fsCachedData";
static const NSInteger kDefaultMaxCacheAge = 7 * 24 * 60 * 60;
static const NSInteger kDefaultMaxCacheSize = 256 * 1024 * 1024;
...
- (void)clearExpiredDiskCache
{
    dispatch_async(_ioQueue, ^{
        ...
        //Remove expired data in the directory of NSURLCache
        NSURL *defaultCacheURL = [NSURL fileURLWithPath:[self defaultCachePath] isDirectory:YES];
        dirEnumerator = [manager enumeratorAtURL:defaultCacheURL includingPropertiesForKeys:URLResourceKeys options:NSDirectoryEnumerationSkipsHiddenFiles errorHandler:nil];
        for (NSURL *fileURL in dirEnumerator) {
            NSDictionary *resourceValues = [fileURL resourceValuesForKeys:URLResourceKeys error:NULL];
            if ([resourceValues[NSURLIsDirectoryKey] boolValue]) {
                continue;
...
- (NSString *)defaultCachePath
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cachePath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:CBDefaultCacheDirName];
    });
    return cachePath;
}
